...
namespace Practice.InterviewBit.Problems.ProblemSet.Level7.GreedyAlgorithm
{
    ...
    public class Seats
    {
        ...
        */
        ...
        private const int MOD = 10000003;
        private const char SEATED = 'x';

        public int Solve(string seats)
        {
            int medianSeat = FindMedianSeat(seats);
            long total = 0;
            var currentCount = 0;
            for (var i = 0; i < medianSeat; i++)
            {
                if (seats[i] == SEATED)
                {
                    currentCount++;
                    total = total + (medianSeat - i) - currentCount;
                    total %= MOD;
                }
            }
            currentCount = 0;
            for (var i = medianSeat + 1; i < seats.Length; i++)
            {
                if (seats[i] == SEATED)
                {
                    currentCount++;
                    total = total + (i - medianSeat) - currentCount;
                    total %= MOD;
                }
            }
            return (int)total;
        ...
        private static int FindMedianSeat(string seats)
        {
            var personCount = 0;
