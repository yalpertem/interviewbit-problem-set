...
namespace Practice.InterviewBit.Problems.ProblemSet.Level7.GreedyAlgorithm
{
    ...
    public class GasStation
    {
        ...
        */
        ...
        public int Solve(List<int> A, List<int> B)
        {
            ...
            for (var i = 0; i < A.Count; i++)
            {
                if (A[i] > B[i])
                {
                    ...
                    while (true)
                    {
                        currentGas = currentGas + A[j] - B[j];
                        if (currentGas < 0)
                        {
                            i = j; //main trick, try searching after last fail
                            break;
                        }
                        j = (j + 1) % A.Count;
                        if (i == j)
        ...
        public bool SolutionExists(List<int> A, List<int> B)
        {
            var gasDifference = 0;
            for (var i = 0; i < A.Count; i++)
            {
                gasDifference += A[i] - B[i];
            }
            return gasDifference > 0;
        }
